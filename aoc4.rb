...
def get_hash(input)
    ...
    guard_hash.each do |key, value|
        count = 0 
        value.each do |arr|
            count += arr.count("asleep")
        end
        minutes_asleep_per_guard[key] = count
    end

    calculate_sleepiest_minute(minutes_asleep_per_guard.max_by{|k,v| v}[0], guard_hash)*minutes_asleep_per_guard.max_by{|k,v| v}[0][1..-1].to_i
end

def calculate_sleepiest_minute(guard, guard_hash)
