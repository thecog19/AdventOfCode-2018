...
def letter_n_times_in_str?(str, n)
	str_arr = str.split('')
	count = {}
	...
	count.any? {|key, value| value == n}
end


def gen_checksum(ids)
	two_times = 0
	three_times = 0
	id_arr = ids.split("\n")
	id_arr.each do |id|
		two_times += 1 if letter_n_times_in_str?(id, 2)
		three_times += 1 if letter_n_times_in_str?(id, 3)
	end
